...
void Bitboards::add_piece(const Color side, const PieceType piece_type,
                          const int at) {
...
void Bitboards::remove_piece(const Color side, const PieceType piece_type,
                             const int at) {
  _position.pieces[side][piece_type] &= ~(1ULL << at);
  update_side(side);
  update_all();
...
void Bitboards::move_piece(const Color side, const PieceType piece_type,
...
PieceType Bitboards::get_piece_type(const Color side, const int at) const {
  uint64_t mask = (1ULL << at);
  for (int i = 0; i < 6; i++) {
    if (_position.pieces[side][i] & mask)
