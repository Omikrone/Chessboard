...
Game::Game()
    : _board(_position), _history(), _zobrist(_position, _board),
...
bool Game::try_apply_move(const int from, const int to,
  ...
  if (piece_color != _position.side_to_move)
    return false; // The player can't play a piece from the other side
  PieceType piece_type = _board.get_piece_type(_position.side_to_move, from);

  // Verifies that the move is legal
  std::vector<Move> moves = MoveGenerator::piece_moves(
      from, _position.side_to_move, piece_type, _position);
  for (Move m : moves) {
...
GameState Game::get_game_state() {
  ...
  if (_position.halfmove_clock >= 50)
    return GameState::DRAW_BY_FIFTY_MOVE_RULE;
  if (_position.halfmove_clock >= 75)
    return GameState::DRAW_BY_75_MOVE_RULE;
  ...
  for (size_t i = 1; i <= _history.size(); i++) {
    ...
    if (_history.at(_history.size() - i).zobrist_hash ==
        _position.zobrist_hash) {
      counter++;
    }
  }
...
void Game::next_turn() {
  _position.side_to_move =
      (_position.side_to_move == Color::WHITE) ? Color::BLACK : Color::WHITE;
}
...
std::vector<Move> Game::get_legal_moves() {
  ...
  std::vector<Move> possible_moves = MoveGenerator::all_possible_moves(
      _position.side_to_move, _position, _board);
