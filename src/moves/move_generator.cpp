...
std::vector<Move> MoveGenerator::all_possible_moves(const Color side,
                                                    ...
                                                    const Bitboards &board) {
  std::vector<Move> moves;
  uint64_t mask;
  for (int i = 0; i < 64; i++) {
    mask = 1ULL << i;

    if ((game_state.colors[side] & mask) == 0)
      continue;
...
std::vector<Move> MoveGenerator::piece_moves(const int square, const Color side,
  ...
  switch (piece_type) {
  case PieceType::PAWN:
    moves = pawn_moves(square, game_state, side, ~game_state.all_pieces,
                       game_state.colors[opponent_side]);
    break;
  case PieceType::BISHOP:
    moves = MoveGenerator::bishop_moves(square, game_state.colors[side],
                                        game_state.colors[opponent_side]);
    break;
  case PieceType::KNIGHT:
    moves = knight_moves(square, game_state.colors[side],
                         game_state.colors[opponent_side]);
    break;
  case PieceType::ROOK:
    moves = rook_moves(square, game_state.colors[side],
                       game_state.colors[opponent_side]);
    break;
  case PieceType::QUEEN:
    moves = queen_moves(square, game_state.colors[side],
                        game_state.colors[opponent_side]);
    break;
  case PieceType::KING:
    moves = king_moves(square, side, game_state.colors[side],
                       game_state.colors[opponent_side],
...
std::vector<Move> MoveGenerator::pawn_moves(const int square,
                                            const Position &game_state,
                                            const Color side,
  ...
  if (side == Color::WHITE) {
      ...
      if (file < 7) {
        ...
        if ((opponent_side >> target) & 1)
          moves.push_back({square, target,
                           (rank == 6 ? MoveType::PROMOTION : MoveType::NORMAL),
                           true});
        else if (game_state.en_passant_square != -1 &&
                 game_state.en_passant_square == target)
    ...
    if (rank > 0) {
      ...
      if (file < 7) {
        ...
        if ((opponent_side >> target) & 1)
          moves.push_back({square, target,
                           (rank == 1 ? MoveType::PROMOTION : MoveType::NORMAL),
                           true});
        else if (game_state.en_passant_square != -1 &&
                 game_state.en_passant_square == target)
          moves.push_back({square, target, MoveType::EN_PASSANT, true});
...
std::vector<Move> MoveGenerator::knight_moves(const int square,
                                              const uint64_t side,
                                              const uint64_t opponent_side) {
  std::vector<Move> moves;
...
std::vector<Move> MoveGenerator::rook_moves(const int square,
                                            const uint64_t side,
                                            const uint64_t opponent_side) {
  std::vector<Move> moves;
  ...
  for (int d : directions) {
    ...
    while (true) {
      ...
      if ((d == 1 || d == -1) && std::abs(toX - fromX) != count)
        break;
...
std::vector<Move> MoveGenerator::bishop_moves(const int square,
                                              const uint64_t side,
                                              const uint64_t opponent_side) {
  std::vector<Move> moves;
  ...
  for (int d : directions) {
    ...
    while (true) {
      ...
      if (std::abs(toX - fromX) != count || std::abs(toY - fromY) != count)
        break;
...
std::vector<Move> MoveGenerator::queen_moves(const int square,
                                             const uint64_t side,
                                             const uint64_t opponent_side) {
  std::vector<Move> bishop_moves =
      MoveGenerator::bishop_moves(square, side, opponent_side);
  std::vector<Move> rook_moves =
      MoveGenerator::rook_moves(square, side, opponent_side);
  bishop_moves.insert(bishop_moves.end(), rook_moves.begin(), rook_moves.end());
  return bishop_moves;
}

std::vector<Move> MoveGenerator::king_moves(const int square,
                                            const Color side_c,
                                            const uint64_t side,
                                            const uint64_t opponent_side,
