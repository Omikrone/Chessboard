...
MoveValidator::MoveValidator(Position &game_state, Bitboards &board,
                             MoveExecutor &executor)
    : _position(game_state), _board(board), _executor(executor) {}

bool MoveValidator::is_square_attacked(const int square,
                                       const Color opponent) const {
  std::vector<Move> enemy_moves =
      MoveGenerator::all_possible_moves(opponent, _position, _board);
...
bool MoveValidator::check_castle(const Move &move,
                                 const Color king_color) const {
  int square = find_king(king_color);
  Color opponent_color =
      (king_color == Color::WHITE) ? Color::BLACK : Color::WHITE;

  if (move.type == MoveType::CASTLE_KINGSIDE &&
      (is_square_attacked(square, opponent_color) ||
